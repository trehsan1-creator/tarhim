...
> تصاویر تزئینی تولیدشده در `assets/` قرار دارند و همه‌ی مسیرهای عکس و صدا محلی هستند. فایل‌های نمونه‌ی موجود برای پیش‌نمایش‌اند؛ عکس و نوای خانوادگی را جایگزین کنید. صدا با `preload="none"` فقط هنگام لمس پاکت درخواست می‌شود تا قبل از تعامل کاربر کل فایل دانلود نشود و پخش به‌صورت progressive انجام شود.
